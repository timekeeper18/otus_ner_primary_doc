...
class Ner:
    ...
    def get_span_coordinates(self):
        ...
        for t in filter(lambda x: x[0] != 'O', self.spans):
            for m in find_near_matches(t[1], self.text.lower().replace('й', 'и').replace('ё','е'), max_l_dist=2):  # , max_deletions=1, max_insertions=1
                if "".join(m.matched.split()).replace('й', 'и').replace('ё','е') == "".join(t[1].split()):
